  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (file.size > 15 * 1024 * 1024) {
        alert("Please upload an image smaller than 15MB. It will be automatically compressed.");
        return;
      }
      const reader = new FileReader();
      reader.onloadend = () => {
        // Auto-compress the image via Canvas
        const img = new Image();
        img.onload = () => {
          ...
          // Compress to JPEG with 80% quality (drastically reduces Base64 size)
